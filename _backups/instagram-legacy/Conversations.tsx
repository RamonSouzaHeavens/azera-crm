        ...
        <div className="flex-1 overflow-hidden flex p-4 gap-4 relative z-10">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        {selectedConversa?.channel === 'instagram' ? (
                          // Se o nome já é o @usuário, não mostra repetido
                          !selectedConversa.contact_name.startsWith('@') && (
                            <span className="font-medium text-[#E4405F]">
                              {`@${selectedConversa.contact_name.replace(/\s+/g, '').toLowerCase()}`}
                            </span>
                          )
                        ) : (
          ...
          <div className={`w-80 rounded-3xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 hidden lg:flex flex-col shadow-sm overflow-hidden`}>
            ...
            <div className="p-6 border-b border-gray-100 dark:border-gray-800 text-center relative">
              ...
              <p className={`text-sm ${selectedConversa?.channel === 'instagram' ? 'text-pink-600 font-medium' : 'text-gray-500'}`}>
                {selectedConversa?.channel === 'instagram'
                  ? (selectedConversa.contact_name.startsWith('@')
                    ? (lead?.email || 'Visitante Instagram')
                    : `@${selectedConversa.contact_name.replace(/\s+/g, '').toLowerCase()}`)
              ...
              {activeTab === 'info' && (
                <div className="space-y-4 animate-in fade-in duration-300">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="group bg-gray-50 dark:bg-slate-800/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-emerald-200 transition-colors">
                      <div className="text-xs text-gray-400 uppercase font-semibold mb-1 flex items-center gap-2">
                        {selectedConversa?.channel === 'instagram' ? <Instagram className="w-3 h-3" /> : <PhoneIcon className="w-3 h-3" />}
                        {selectedConversa?.channel === 'instagram' ? 'Instagram ID' : t('conversations.phone')}
                      </div>
                      <div className="flex items-center justify-between">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              // 1. Diagnóstico: Verificar se o registro existe e é acessível
                              ...
                              // 2. Diagnóstico: Verificar Membership do usuário
                              if (user?.id && currentTenantId) {
                                ...
                                if (memberError || !memberData) {
                                  console.warn('[LeadUpdate] AVISO: Membership não encontrado ou erro.', memberError);
                                } else {
                                  console.log('[LeadUpdate] Membership confirmado:', memberData);
                                }
                              }

                              // 3. Tentativa real de update
                              // IMPORTANTE: Incluindo tenant_id para garantir que a policy CHECK tenha o valor explícito
                              const { data, error } = await supabase
                                .from('clientes')
                                .update({
                                  status: newStatusKey,
                                  tenant_id: currentTenantId // Reenviando tenant_id para satisfazer RLS CHECK
                                })
                                .eq('id', lead.id)
                                .select();
                              ...
                              if (!data || data.length === 0) {
                                ...
                                console.log('[LeadUpdate] Tentativa feita com tenant_id explícito:', currentTenantId);
